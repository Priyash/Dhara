...
function ApprovalArc({ rate }) {
  ...
  const color   = rate >= 70 ? '#4ade80' : rate >= 40 ? '#f07a4e' : '#f87171'
  return (
...
function StatusDonut({ counts }) {
  const segs = [
    ...
    { label: 'In Review', value: counts.pending  || 0, color: '#f07a4e' },
...
function PipelineFlow({ counts, content }) {
  ...
  const stages = [
    ...
    { label: 'Has Likes', value: withLikes,            color: '#e85d26', bg: 'rgba(232,93,38,0.08)'  },
  ]
  return (
...
function RankingBars({ items }) {
  ...
  const GRAD   = { Film: ['#4c1d95','#a78bfa'], Series: ['#78350f','#e85d26'], Documentary: ['#064e3b','#34d399'] }
  return (
    <div className={styles.rankingBars}>
      {sorted.map((item, i) => {
        ...
        const eColor = Number(eng) >= 10 ? '#4ade80' : Number(eng) >= 4 ? '#f07a4e' : 'rgba(255,255,255,0.25)'
        return (
...
function EngagementScatter({ items }) {
  ...
  const TC   = { Film: '#a78bfa', Series: '#e85d26', Documentary: '#34d399' }
...
function calcHealth(item, avgViews) {
  ...
  if (score >= 70) return { label: 'Hot',     color: '#e85d26', Icon: Flame        }
...
function InsightsRow({ overview, content }) {
  ...
  if (ov.topContent && ov.avgViewsPerTitle > 0) {
    ...
    if (Number(mult) >= 1.5)
      cards.push({ color: '#e85d26', text: `"${ov.topContent.title}" pulls ${mult}× more views than your average title` })
  }
          ...
          return (
            <g key={d.hour}>
              <rect x={x} y={y} width={barW} height={barH} rx="2"
                fill={isPk ? '#e85d26' : hasAny && d.views > 0 ? 'rgba(167,139,250,0.5)' : 'rgba(255,255,255,0.07)'}
                style={isPk ? { filter: 'drop-shadow(0 0 4px rgba(232,93,38,0.55))' } : undefined} />
              {show && (
                <text x={cx} y={H - 1} textAnchor="middle" fontSize="8"
                  fill={isPk ? 'rgba(232,93,38,0.9)' : 'rgba(255,255,255,0.22)'}
...
const STATUS_META = {
  pending:  { label: 'In Review',  color: '#f07a4e', bg: 'rgba(240,122,78,0.12)'  },
            ...
            {analyticsSubTab === 'content' && (analyticsLoading ? (
              ...
              <>
                ...
                <div className={styles.analyticsStatRow}>
                  ...
                  <div className={styles.analyticsStatCard}>
                    <div className={styles.analyticsStatIcon} style={{ color: '#e85d26', background: 'rgba(232,93,38,0.1)' }}>
                      <ThumbsUp size={16} />
                    </div>
                    ...
                    <div className={styles.analyticsContentList}>
                      {content
                        ...
                        .map((item, rank) => {
                          ...
                          const eColor      = likePct >= 10 ? '#4ade80' : likePct >= 4 ? '#f07a4e' : 'rgba(255,255,255,0.28)'
                          ...
                          return (
                            <div key={item._id} className={styles.analyticsContentRow}>
                              ...
                              <div className={styles.analyticsContentBody}>
                                ...
                                <div className={styles.analyticsDualBarRow}>
                                  <ThumbsUp size={10} style={{ color: '#e85d26', flexShrink: 0, opacity: 0.7 }} />
                                  <div className={styles.analyticsDualBarTrack}>
              ...
              <>
                ...
                {(() => {
                  ...
                  if (topReel && (totalViews || 0) > 0) {
                    ...
                    if (Number(mult) >= 1.5)
                      cards.push({ color: '#e85d26', text: `"${topReel.title || 'Your top reel'}" gets ${mult}× more views than your average reel` })
                  }
                  if (peakHour != null) {
                      ...
                      <div className={styles.reelHealthGrid}>
                        {approved.sort((a, b) => (b.viewCount || 0) - (a.viewCount || 0)).map((reel) => {
                          ...
                          const health = score >= 70
                            ? { label: 'Hot',     color: '#e85d26', bg: 'rgba(232,93,38,0.1)',  Icon: Flame }
                            : score >= 45
                    ...
                    {[...reels].sort((a, b) => (b.viewCount||0) - (a.viewCount||0)).map((reel) => {
                      ...
                      return (
                        <div key={reel._id} className={styles.reelAnalyticsRow}>
                          ...
                          <span className={styles.reelAnalyticsStat} style={{ color: '#e85d26' }}>{fmtReel(likes)}</span>
                      ...
                      {payouts.map((payout) => (
                        <div key={payout._id} className={styles.revenuePayoutItem}>
                          ...
                          <span className={styles.revenuePayoutStatus} style={{
                            color:       payout.status === 'paid' ? '#4ade80' : '#f07a4e',
                            background:  payout.status === 'paid' ? 'rgba(74,222,128,0.1)' : 'rgba(240,122,78,0.1)',
                            borderColor: payout.status === 'paid' ? 'rgba(74,222,128,0.25)' : 'rgba(240,122,78,0.25)',
                          }}>
                  ...
                  <div className={styles.rowTop}>
                    ...
                    {item.revisionCount > 0 && (
                      <span style={{
                        ...
                        background: item.revisionCount >= 4 ? 'rgba(248,113,113,0.12)' : 'rgba(240,122,78,0.10)',
                        color:      item.revisionCount >= 4 ? '#f87171' : '#f07a4e',
                        border:     `1px solid ${item.revisionCount >= 4 ? 'rgba(248,113,113,0.25)' : 'rgba(240,122,78,0.2)'}`,
                        flexShrink: 0,
                      }}>
